...
function calculateDiscount() {
    const price = parseFloat(prompt('Ingrese el precio:')); // Convierte el input del precio a número
    const quantity = parseInt(prompt('Ingrese la cantidad:'), 10); // Convierte el input de la cantidad a entero

    if (!isNaN(price) && !isNaN(quantity)) { // Verifica que los valores sean números válidos
        const total = price * quantity; // Calcula el precio total
        const discount = total * 0.25; // Calcula el descuento del 25%
        const finalPrice = total - discount; // Calculam el precio final con el descuento
